...
class sdrsssyndicatorViewFeed extends JView
{
	function display($tpl = null)
	{
		...
		$fulltext[] = JHTML::_('select.option', "5","(только для Яндекса и Рамблера) Введение + основная статья");
		$fulltext[] = JHTML::_('select.option', "4","(только для Яндекса и Рамблера) только полный текст без введения");
		...
		$lists['fulltextlist'] = JHTML::_('select.genericlist', $fulltext, 'msg_fulltext', 'class="inputbox"', 'value', 'text',  $isNew ? '1': $feed->msg_fulltext );
                ...
                $shURLs[] = JHTML::_('select.option', "0","не использовать ссылки shURL");
		$shURLs[] = JHTML::_('select.option', "1","использовать ссылки shURL");


		$lists['shURL'] = JHTML::_('select.genericlist', $shURLs, 'shURLs', 'class="inputbox"', 'value', 'text',  $isNew ? '0': $feed->shURL );
    ...
    $FPItemsOnly[] = JHTML::_( 'select.option', '1','Статьи опубликованные на главной');
	  ...
	  $lists['renderPublishedList'] = JHTML::_( 'select.genericList', $yesNoList, 'published', 'class="inputbox"','value', 'text',$isNew ? NULL : $feed->published);
		...
		$rssfeed_categoryItem[] = JHTML::_('select.option', "1", "Раздел Joomla");
