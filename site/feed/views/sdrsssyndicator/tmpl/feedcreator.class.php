...
/**
	...
	function createFeed() {
		...
		for ($i=0;$i<count($this->items);$i++) {
			$feed.= "        <item>\n";
			$feed.= "            <title>".strip_tags($this->items[$i]->title)."</title>\n";
			$feed.= "            <link>".htmlspecialchars($this->items[$i]->link)."</link>\n";
			// если будет нужен <pdalink> то следующюю строку раскоментируем
			//$feed.= "            <pdalink>".htmlspecialchars($this->items[$i]->link)."</pdalink>\n";
			
			
			$feed.= "            <description>".htmlspecialchars( strip_tags($this->items[$i]->description))."</description>\n";
			...
			*/
			...
			$feed.= "				<yandex:full-text>".htmlspecialchars( strip_tags(($this->items[$i]->descriptionfull),'<p>' ))."</yandex:full-text>\n";
